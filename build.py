...
SECTIONS = [
    ...
    ('二、自动求导（Autograd）— PyTorch 灵魂', None, []),
...
cells.append(new_markdown_cell('### 练习：backward 验证'))
cells.append(new_code_cell("""import torch

x = torch.tensor([1., 2., 3.], requires_grad=True)
y = (x ** 2).sum()  # y = 1+4+9=14
y.backward()
print("x.grad:", x.grad)  # tensor([2., 4., 6.]) = 2x
...
for i in range(3):
    ...
    y = (x ** 2).sum()
    y.backward()
    print(f"第{i+1}次 backward, grad:", x.grad)"""))

cells.append(new_markdown_cell('### 练习：detach 截断梯度'))
cells.append(new_code_cell("""import torch
...
cells.append(new_markdown_cell('### 练习：梯度累积机制（关键！）'))
cells.append(new_code_cell("""import torch
...
for _ in range(3):
    y = (x ** 2).sum()
    y.backward()
    print(x.grad)  # 每次都累加，越来越大

# 正确做法：每次迭代前 zero_grad
print("\\n--- 正确做法 ---")
x = torch.tensor([1., 2., 3.], requires_grad=True)
...
out = os.path.join(DIR, 'W1D1-PyTorch-学习笔记.ipynb')
